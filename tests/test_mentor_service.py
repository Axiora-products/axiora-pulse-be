...
def test_build_mentor_system_prompt_includes_domain_scope_and_off_topic_guardrails():
    prompt = _build_mentor_system_prompt(
        workspace_id="ws-789",
        state="GATHERING_INFO",
        idea_json=json.dumps({"idea_title": "CleanEnergy SaaS"}),
        missing_fields="Target Customer",
    )
    assert "STRICT DOMAIN SCOPE & OFF-TOPIC REDIRECTION GUARDRAILS" in prompt
    assert "You are EXCLUSIVELY the Axiora Pulse AI Mentor & Co-Founder" in prompt
    assert "OFF-TOPIC HANDLING (STRICT):" in prompt
    assert "Politely decline in ONE short sentence" in prompt


def test_extract_system_prompt_includes_off_topic_filtering():
    from app.services.mentor_service import EXTRACT_SYSTEM_PROMPT
    assert "OFF-TOPIC & CASUAL BANTER FILTERING (CRITICAL):" in EXTRACT_SYSTEM_PROMPT
    assert "NEVER extract off-topic subject matter into idea_title" in EXTRACT_SYSTEM_PROMPT
