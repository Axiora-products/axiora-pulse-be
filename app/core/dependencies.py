...
async def require_active_subscription(
    ...
    """
    Gate a route behind a paid, entitled subscription.

    Admins bypass the check. Everyone else must hold a subscription in an entitled
    state (active/authenticated) as tracked by the Razorpay webhooks. Apply to the
    paid surfaces, e.g.:
    ...
    # which this module also references at startup).
    from app.services.billing_service import billing_service

    if not await billing_service.has_active_entitlement(current_user, db):
        raise HTTPException(
...
async def require_export_enabled(
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
) -> User:
    """Gate a route behind a plan that includes report export (Builder / Pro).

    Authenticates like ``get_current_user`` and additionally enforces the per-plan
    ``export_enabled`` limit. Apply to the export routes, e.g.:

        @router.get("/{id}/export")
        async def export(user: User = Depends(require_export_enabled)):
            ...
    """
    # Imported here to avoid a circular import (services import from db/models,
    # which this module also references at startup).
    ...
    await billing_service.assert_export_allowed(current_user, db)
    return current_user
